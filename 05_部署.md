...
## 尝试改进缓存
https://e5.sakiko.top/user-list 头像的图片缓存还是有问题
- 好像是因为Etag导致的缓存不稳定？禁用一下吧
- 另外设置一下public比较好，表示响应可以被公共缓存机制（如代理服务器、CDN 等）缓存。
- 使用 `expires` 指令时，Nginx 会自动在响应中生成 `Cache-Control` 头部，并根据指定的过期时间生成相应的 `max-age` 值。
```nginx
