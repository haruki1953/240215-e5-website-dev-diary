...
## 使用 jimp 压缩图片
【240418】由于sharp导致在部署时`Illegal instruction (core dumped)`，更换图片处理模块为jimp
安装 jimp：pnpm add jimp
卸载 sharp：pnpm uninstall sharp
...
【240417】通过环境变量来配置监听端口
```js
const port = process.env.E5SHARE_NODE_PORT || 23769;
// E5SHARE_NODE_PORT=23769 node app.js
```
默认端口暂定为`23769`，项目名每个字符的 ASCII 值相乘，取前五位83769，从左到右取模直到可用
...
/*
export E5SHARE_NODE_JWT_SECRET_KEY="custom_secret_key"
export E5SHARE_NODE_ADMIN_JWT_SECRET_KEY="admin_custom_secret_key"
*/
```

【240418】由于sharp导致在部署时`Illegal instruction (core dumped)`，更换图片处理模块为jimp

【240418】为了方便部署，JWT 随机生成并保存至 utils/jwtKeys.json

【TODO】发现自己在打包node后端时好像并没有区分 开发环境 与 生产环境，在学hono时要注意这个一起了解这个